  ...
  <div>
    <v-container>
      <v-row>
        ...
        <v-col cols="12" sm="8" v-for="persona in personagem" :key="persona.id">
          <h1 class="grey--text text--darken-3 mt-5">{{ persona.name }}</h1>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getPersonagem", this.$route.params.id);
  ...
  computed: {
    ...mapState({
      personagem: (state) => state.personagem,
